...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    ...
    $stmt = $conn->prepare("
        ...
        FROM Appointment a
        WHERE a.ID = ? AND a.Customer_ID = ? AND (a.status = 'pending' OR a.status = 'price_quoted')
    ");
    ...
    if ($result->num_rows === 0) {
        $error = "Appointment not found or cannot be cancelled at this stage.";
    } else {
        ...
        if ($stmt->execute()) {
            ...
            $error = "Error cancelling appointment: " . $stmt->error;
        }
